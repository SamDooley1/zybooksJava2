/*
Given integer inputs howMany and maxNum, generate an array of howMany unique 
random integers from 0 to maxNum (exclusive).
...
Complete uniqueRandomInts(), which generates random integers until howMany unique
integers have been collected in array nums.

Hint: If a generated number is new, add the number to the array nums and the set 
alreadySeen. If the number has been seen before, increment the static variable 
retries and generate another random integer.

Note: For testing purposes, a random number generator object is created with a 
fixed seed value (29) in uniqueRandomInts(). Refer to the textbook section on 
...
public class LabProgram {
   ...
   public static int[] uniqueRandomInts(int howMany, int maxNum) {
      ...
      /* Type your code here. */
      while (i < howMany) {
         randInt = randGen.nextInt(maxNum);
         if (!alreadySeen.contains(randInt)) {
            nums[i] = randInt;
